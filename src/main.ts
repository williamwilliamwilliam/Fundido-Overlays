...
function setupCaptureToOverlayPipeline(): void {
    captureService.setFrameCapturedCallback((frame) => {
        ...
        // When the UI is minimized (user is gaming), only evaluate regions that
        // are actually referenced by enabled overlay groups. This avoids wasting
        // CPU/GPU on regions that exist only for configuration previewing.
        let monitoredRegions = allMonitoredRegions;
        if (uiMinimizedRef.minimized) {
            const referencedRegionIds = getRegionIdsReferencedByEnabledOverlays();
            monitoredRegions = allMonitoredRegions.filter(
        ...
        // (skip when UI is minimized to save IPC overhead)
